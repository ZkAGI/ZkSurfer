...
describe('parseResponse', () => {
  ...
  it("Should get values as response", () => {
    expect(
      parseResponse(
        `<Thought>Click the "Sign Up" button</Thought>\n<Action>finish()</Action>`
      )
  ...
  it('should parse a response for scrapeValue action', () => {
    expect(
      parseResponse(
        `<Thought>Scrape the value of an element</Thought>\n<Action>scrapeValue(123)</Action>`
      )
    ).toEqual({
      thought: 'Scrape the value of an element',
      action: 'scrapeValue(123)',
      parsedAction: {
        name: 'scrapeValue',
        args: {
          elementId: 123,
