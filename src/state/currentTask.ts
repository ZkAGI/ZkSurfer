    ...
    runTask: async (onError) => {
      ...
      try {
        ...
        while (true) {
          ...
          if (action.parsedAction.name === 'scrapeValue') {
            const { elementId, elementvalue } = action.parsedAction.args as {
              elementId: number;
              elementvalue: string;
            };
            await callDOMAction('scrapeValue', { elementId, elementValue: elementvalue });
          }
